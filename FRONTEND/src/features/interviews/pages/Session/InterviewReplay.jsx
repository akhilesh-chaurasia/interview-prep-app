...
const ReplayItem = ({ question, index, isOpen, onToggle }) => {
    const headerId = `replay-header-${index}`
    const bodyId = `replay-body-${index}`
    return (
        <div className={`replay-item ${isOpen ? 'replay-item--open' : ''}`}>
            ...
            >
                <div className='replay-item__meta'>
                    <span className={`replay-item__index ${question.answer ? 'replay-item__index--answered' : ''}`}>
                        Q{index + 1}
                    </span>
            ...
            <div id={bodyId} className='replay-item__body' aria-labelledby={headerId}>
                <div className='replay-item__inner'>
                    ...
                    {question.missingPoints?.length > 0 && (
                        <div className='replay-section replay-section--missing'>
                            <span className='replay-section__label'>⚠️ Missing Points</span>
                    ...
                    {/* Rubric Breakdown */}
                    {question.rubric && (
                        <RubricBreakdown rubric={question.rubric} />
                    )}
                ...
                <div className='replay-card__title-row'>
                    ...
                    <div>
                        ...
                        <p className='replay-card__subtitle'>
                            <strong>{answered.length}</strong> of <strong>{questions.length}</strong> questions answered
                        </p>
                    </div>
                </div>
            </div>
